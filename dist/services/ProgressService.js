...
class ProgressService {
    static async openContent(studentId, contentId) {
        ...
        if (!content) {
            throw new errors_1.NotFoundError('Content not found or not approved');
        }
    ...
    static async completeContent(studentId, contentId, quizScore) {
        const progress = await Progress_1.Progress.findOne({ studentId, contentId });
        if (!progress) {
            throw new errors_1.NotFoundError('Progress not found. Please open the content first.');
        }
        progress.status = 'completed';
        progress.completedAt = new Date();
        if (quizScore !== undefined) {
            if (quizScore < 0 || quizScore > 100) {
                throw new errors_1.ValidationError('Quiz score must be between 0 and 100');
            }
    ...
    static async recordVideoTime(studentId, contentId, secondsSinceLastPing) {
    ...
    static async getStudentProgress(studentId, recentLimit, watchLimit, classId, subjectId) {
        // timezone for dateTrunc
        const TZ = "Asia/Kolkata";
        // Build pipeline
        const pipeline = [];
        // 1) base match by student
        pipeline.push({ $match: { studentId: new mongoose_1.default.Types.ObjectId(String(studentId)) } });
        ...
        if (classId) {
            pipeline.push({
                $match: {
                    $expr: {
                        $or: [
                            { $eq: ["$class._id", new mongoose_1.default.Types.ObjectId(String(classId))] },
                            { $eq: ["$_classIdFromContent", new mongoose_1.default.Types.ObjectId(String(classId))] },
                        ],
                    },
                },
            });
        ...
        if (subjectId) {
            pipeline.push({
                $match: {
                    $expr: {
                        $or: [
                            { $eq: ["$subject._id", new mongoose_1.default.Types.ObjectId(String(subjectId))] },
                            { $eq: ["$_subjectIdFromContent", new mongoose_1.default.Types.ObjectId(String(subjectId))] },
                        ],
                    },
                },
            });
        ...
        // run aggregation
        const aggResult = (await Progress_1.Progress.aggregate(pipeline).allowDiskUse(true).exec());
        ...
        // classes
        const classesOut = (faceted.classes || []).map((c) => {
        ...
        const watchHistory = (faceted.watchHistory || []).map((w) => ({ ...w, progress: Math.round((w.progress || 0) * 10) / 10 }));
        // weekly & monthly buckets - convert date objects to ISO strings (or keep Date objects if preferred)
        const weekly = (faceted.weeklyBuckets || []).map((b) => ({
        ...
        const monthly = (faceted.monthlyBuckets || []).map((b) => ({
