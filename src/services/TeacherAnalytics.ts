...
export class TeacherAnalyticsService {
  static async getTeacherAnalytics(teacherId: mongoose.Types.ObjectId) {
    const now = new Date();
    ...
    /* ===========================
       EXECUTION
    ============================ */
    const [viewAgg] = await ContentView.aggregate(viewPipeline)
      .allowDiskUse(true)
      .exec();

    const [contentAgg] = await Content.aggregate(contentPipeline)
      .allowDiskUse(true)
      .exec();
    ...
    return {
      ...
      contentHealth: {
        ...
        avgViewsPerContent:
          totalContent > 0 ? Math.round(allTime.totalViews / totalContent) : 0,
      },
